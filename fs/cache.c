	...
	for (cou += sec; sec < cou; sec++, buf += BLK_SIZ)	/*cou用做结束块ID*/
	{
		...
		else	/*缓冲区中不存在就要访问设备*/
		{
			...
			if (CurBmd->BlkID & CACHE_ATTR_DIRTY)	/*当前缓冲块脏,不可丢弃,扫描可预先保存的脏块*/
			{
				...
				if ((res = HDWriteSector(AthdPtid, CurBmd->DrvID, (CurBmd->BlkID & BLKID_MASK) | (sec & BLKATTR_MASK), PreBmd - CurBmd, cachep)) != NO_ERROR)	/*保存脏块*/
				{
					ulock(&cahl);
					return res;
				}
			...
			else	/*读操作,读入缓存并复制*/
			{
				...
				if ((res = HDReadSector(AthdPtid, drv, (sec & BLKID_MASK) | (sec & BLKATTR_MASK), PreBmd - CurBmd, cachep)) != NO_ERROR)	/*预读取*/
				{
					ulock(&cahl);
					return res;
				}
...
long SaveCache()
{
	...
	for (CurBmd = bmt; CurBmd < &bmt[BMT_LEN];)
	{
		if (CurBmd->BlkID & CACHE_ATTR_DIRTY)
		{
			for (PreBmd = CurBmd + 1; PreBmd - CurBmd < MAXPROC_COU && PreBmd < &bmt[BMT_LEN] && (PreBmd->BlkID & CACHE_ATTR_DIRTY) && (PreBmd->BlkID >> BLKID_SHIFT) == (CurBmd->BlkID >> BLKID_SHIFT) && PreBmd->DrvID == CurBmd->DrvID; PreBmd++)
				PreBmd->BlkID &= ~CACHE_ATTR_DIRTY;
			if ((res = HDWriteSector(AthdPtid, CurBmd->DrvID, (CurBmd->BlkID & BLKID_MASK) | (CurBmd - bmt), PreBmd - CurBmd, cache + ((CurBmd - bmt) << BLK_SHIFT))) != NO_ERROR)	/*保存脏块*/
			{
				ulock(&cahl);
				return res;
			}
